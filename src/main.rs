...
//! (none yet — P1), owns app storage (`--data DIR/{sys,files,config,state}`), and
...
fn main() {
    ...
    let _application_store = match open_application_store(&cfg.application_store_path(), &now) {
        Ok(store) => store,
        Err(error) => {
            eprintln!(
                "[grazel] cannot open application store at {}: {error}",
                cfg.application_store_path().display()
            );
            std::process::exit(2);
        }
    };
    println!(
        "[grazel] application store: {} (generated by garns)",
        cfg.application_store_path().display()
    );
